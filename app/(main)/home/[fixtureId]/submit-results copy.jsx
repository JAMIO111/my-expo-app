...
const SubmitResultsScreen = () => {
  ...
  const handleToggleBonus = (value) => {
    if (value && existingBonusFrame) {
      Toast.show({
        ...
        text2: `Only one ${fixtureDetails?.competition?.special_match_name || 'special'}) frame is allowed per fixture. Please disable the existing bonus frame before enabling another one.`,
      });
      return;
    }
  ...
  const handleEscalate = async () => {
    ...
    try {
      ...
      console.error('Failed to escalate fixture:', error.message);
      Toast.show({
        type: 'error',
        text1: 'Error',
      ...
      isAwayTeamPlayersLoading ? (
        ...
        <View className="flex-1 bg-bg-1">
          <ScrollView
            ...
            className="mt-16 flex-1 bg-bg-grouped-1 px-3">
            {/* Match Score */}
                            ...
                            onPress={() => {
                              if (bestOf && normalFrameCount > bestOf && !frame.bonusFrame) {
                                Toast.show({
                                  type: 'error',
                                  text1: 'Frame Limit Reached',
                                  text2: `This competition is a best of ${bestOf} frames, so you cannot have more than ${bestOf} normal frames. Consider marking this frame as a bonus frame if it's a special match.`,
                                });
                                return;
                              }
                              if (
                          ...
                          <View className="mt-3 flex-col items-center justify-center gap-3">
                            <View className="flex-row items-center justify-evenly">
                              <Pressable
                                ...
                                className={`h-15 w-15 items-center justify-center rounded-xl border ${
                            ...
                            <View className="mt-2 flex-row items-center justify-between gap-5 border-t border-theme-gray-4 bg-bg-1 pt-3">
                              ...
                              <Switch
                                ...
                                onValueChange={(value) => handleToggleBonus(value)}
